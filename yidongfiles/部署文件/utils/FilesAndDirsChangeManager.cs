...
namespace utils.filetest
{
    ...
    public class FilesAndDirsChangeManager
    {
        ...
        /// <param name="isCreate"></param>
        /// <returns></returns>
        public static Boolean hasSomeFile(string filePath,bool isCreate = false)
        {
            var boolens = true;
            hasDirectory(System.IO.Path.GetDirectoryName(filePath), isCreate);
           ...
           System.IO.DirectoryInfo[] subDirs = root.GetDirectories();
            ...
            foreach (System.IO.DirectoryInfo dirInfo in subDirs)
            {
                ...
                if(isMove)
                {
                    ...
                    {
                        Console.WriteLine(e.Message);
                        Log.warn("文件夹已存在或不能被删除:" + targetPath);
        ...
        public static bool copyFiles(string sourcePath, string targetPath, bool isMove = false)
        {
            bool tempbool = false;
            if (!isMove)
            {
                ...
                {
                    Log.warn("没有找到文件夹:" + e.Message);
                    Console.WriteLine(e.Message);
                }catch(System.IO.FileNotFoundException e)
                {
                    Log.warn("没有找到文件:" + e.Message);
                    Console.WriteLine(e.Message);
                }
            }
            else
            {
                ...
                {
                    Console.WriteLine(e.Message);
                    Log.warn("文件已存在或不能被删除:" + targetPath);
        ...
        /// <param name="targetPath"></param>
        /// <returns></returns>
        public static bool renameFiles(string sourcePath,string targetPath)
        {
            bool tempbln = false;
            try
            {
        ...
        public static List<string> findFilePath()
        {
